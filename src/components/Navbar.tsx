...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleAdminPasscodeSubmit = (e: React.FormEvent) => {
    ...
    if (validCodes.includes(adminPasscode.trim())) {
      onUnlockAdmin();
      setShowAdminAuthModal(false);
      setAdminPasscode('');
      setAdminPasscodeError('');
    } else {
      setAdminPasscodeError('Invalid Admin Passcode! Access Denied.');
    }
  };
  ...
  return (
    <header className={`sticky top-0 z-40 backdrop-blur-xl border-b shadow-xl transition-all ${
      activeTab === 'admin'
        ? 'bg-[#0d041a]/95 border-rose-500/30'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between h-16 gap-3">
          ...
          {activeTab === 'admin' ? (
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-rose-500 to-purple-600 p-0.5 shadow-lg shadow-rose-500/30">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  <span className="font-serif text-lg font-bold bg-gradient-to-r from-rose-100 via-amber-100 to-purple-200 bg-clip-text text-transparent">
                    AstroVeda Admin
                  </span>
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-rose-500/30 text-rose-300 border border-rose-500/50 flex items-center gap-1 shadow-sm">
                    <ShieldCheck className="w-3 h-3 text-rose-400" />
                    <span>Control Center</span>
                  </span>
                </div>
                <p className="text-[10px] text-rose-200/70 hidden sm:block">Executive Astrologer & System Management Console</p>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-3 cursor-pointer" onClick={handleLogoClick}>
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 via-purple-600 to-amber-400 p-0.5 shadow-lg shadow-indigo-500/40 overflow-hidden group">
                <img
                  src={astroVedaLogo}
                  alt="AstroVeda AI Logo"
            ...
            {activeTab === 'admin' ? (
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/40 text-rose-200 transition-all text-xs font-bold cursor-pointer shadow-md"
                  title="Grant minutes to any user ID"
                >
                  <ShieldCheck className="w-4 h-4 text-rose-400" />
                  <span className="hidden sm:inline">Recharge User ID</span>
                </button>

                <button
                  onClick={onLockAdmin}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-full bg-indigo-600/30 hover:bg-indigo-600/50 border border-indigo-500/40 text-indigo-200 transition-all text-xs font-bold cursor-pointer"
                  title="Exit Admin & Return to User Portal"
                >
                ...
                <button
                  onClick={onLockAdmin}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-full bg-white/10 hover:bg-rose-950/60 text-gray-300 hover:text-rose-200 transition-all text-xs font-bold border border-white/10 hover:border-rose-500/30 cursor-pointer"
        ...
        {/* Navigation Tabs Bar */}
        {activeTab === 'admin' ? (
          <nav className="flex items-center justify-between gap-2 overflow-x-auto py-2.5 border-t border-rose-500/20 scrollbar-none text-xs">
            <div className="flex items-center gap-2">
              <span className="px-3 py-1.5 rounded-full bg-rose-500/20 text-rose-300 font-bold border border-rose-500/30 flex items-center gap-1.5">
                <ShieldCheck className="w-3.5 h-3.5 text-rose-400" />
                <span>Executive Command Console</span>
              </span>
              <button
                onClick={() => onOpenRechargeModal('ADMIN_GRANT')}
                className="px-3.5 py-1.5 rounded-full bg-white/10 hover:bg-white/15 text-white font-medium flex items-center gap-1.5 transition-all cursor-pointer"
              >
                <PlusCircle className="w-3.5 h-3.5 text-emerald-400" />
                <span>Quick Minute Grant</span>
              </button>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={onLockAdmin}
                className="px-3.5 py-1.5 rounded-full bg-indigo-600/30 hover:bg-indigo-600/50 text-indigo-200 border border-indigo-400/30 font-bold flex items-center gap-1.5 transition-all cursor-pointer"
              >
                <User className="w-3.5 h-3.5 text-indigo-300" />
          ...
          <nav className="flex items-center gap-1.5 overflow-x-auto py-2.5 border-t border-white/10 scrollbar-none text-xs">
            {[
              ...
              ...(isAdminUnlocked ? [{ id: 'admin', label: 'Admin Panel', icon: Lock }] : [])
            ].map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  id={`tab-${tab.id}`}
                  onClick={() => {
                    if (tab.id !== 'admin' && activeTab === 'admin') {
                      onLockAdmin();
                    } else {
                      setActiveTab(tab.id);
                    }
          ...
          <div className="bg-[#0f0926] border border-white/20 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-5 text-left relative overflow-hidden">
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-white text-base font-serif">Admin Mode Unlock</h3>
              </div>
              <button
            ...
            <p className="text-xs text-gray-300">
              Aapne AstroVeda Admin Panel access attempt kiya hai. Kripya apna Secret Admin Security Passcode enter karein:
            </p>

            <form onSubmit={handleAdminPasscodeSubmit} className="space-y-4">
              <div>
                <label className="text-xs text-gray-400 block mb-1">Admin Passcode / Security Key</label>
                <input
                  type="password"
                  value={adminPasscode}
                  onChange={(e) => {
                  ...
                  placeholder="Enter Admin Passcode (e.g. 8800 or 880101)"
                  className="w-full px-3 py-2 bg-black/50 border border-white/10 rounded-xl text-xs text-white focus:outline-none focus:border-rose-500 font-mono"
                  autoFocus
                  required
                />
              </div>
              ...
              {adminPasscodeError && (
                <div className="p-2.5 rounded-xl bg-rose-500/20 border border-rose-500/30 text-xs text-rose-200 font-bold">
                  {adminPasscodeError}
                </div>
              )}
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-rose-600 to-purple-600 text-white font-bold text-xs shadow-lg shadow-rose-600/30 hover:brightness-110 cursor-pointer"
                >
                  Unlock Admin Panel
                </button>
              </div>
